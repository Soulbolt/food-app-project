...
async def test_create_restaurant(client):
    ...
    response = await client.post("/api/new_restaurant", json=new_restaurant)
...
@pytest.mark.asyncio
async def test_get_restaurants_not_found(client):
    response = await client.get("/api/restaurants/10")
    print("data: ", response.json()) # Print the response for debugging
...
@pytest.mark.asyncio
async def test_get_restaurants_by_name(client, name="golden"):
    response = await client.get(f"/api/restaurants_by_name/{name}")
    print("data: ", response.json()) # Print the response for debugging
    assert response.status_code == 200
    assert len(response.json()) > 0 # Check if the response is not empty
...
@pytest.mark.asyncio
async def test_get_restaurants_by_name_not_found(client, name="not found"):
    response = await client.get(f"/api/restaurants_by_name/{name}")
...
async def test_get_restaurant_by_id(client):
    response = await client.get("/api/restaurant/1")
    print("data: ", response.json()) # Print the response for debugging
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_get_recommended_restaurants(client):
    response = await client.get("/api/restaurants/recommended")
    print("data: ", response.json()) # Print the response for debugging
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_update_restaurant(client, id=1):
    update_restaurant = {
        "category": "Mexican",
    ...
    response = await client.patch(f"/api/update_restaurant/{id}", json=update_restaurant)
    print("data: ", response.json()) # Print the response for debugging
    assert response.status_code == 200

    # Verify that the restaurant was updated in the database
    response = await client.get(f"/api/restaurant/{id}")
...
@pytest.mark.asyncio
async def test_delete_restaurant(client, id=103):
    response = await client.delete(f"/api/delete_restaurant/{id}")
    print("data: ", response.json()) # Print the response for debugging
    assert response.status_code == 200
